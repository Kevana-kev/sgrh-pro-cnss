...
from app.models import Attendance, Contract, Department, Employee, EmployeeStatus, Leave, Message, Payroll, Permission, Role, User, Training, TrainingEnrollment, PerformanceEvaluation, JobOffer, JobApplication, Notification
...
class EnterpriseService:
    ...
    def dashboard_overview() -> dict:
        """Retourne toutes les statistiques nécessaires pour le tableau de bord."""
        today = date.today()
        first_day_month = today.replace(day=1)
        last_12_months = [(today.replace(day=1) - timedelta(days=30 * i)).strftime("%Y-%m") for i in range(11, -1, -1)]

        # ---- Effectifs ----
        all_employees = Employee.query.all()
        total_employees = len(all_employees)
        active_employees = sum(1 for e in all_employees if _is_active_employee_status(e.status))
        new_this_month = sum(1 for e in all_employees if e.hire_date and e.hire_date >= first_day_month)

        # Gender repartition
        gender_counts = {}
        for e in all_employees:
            g = getattr(e, "gender", None) or "N/A"
            gender_counts[g] = gender_counts.get(g, 0) + 1

        # ---- Paie ----
        payrolls = Payroll.query.all()
        total_payroll = sum(float(p.net_salary or 0) for p in payrolls)
        avg_salary = round(total_payroll / len(payrolls), 2) if payrolls else 0

        # Masse salariale par mois (12 derniers mois)
        payroll_by_month: dict[str, float] = {m: 0.0 for m in last_12_months}
        for p in payrolls:
            if p.paid_at:
                mk = p.paid_at.strftime("%Y-%m")
                if mk in payroll_by_month:
                    payroll_by_month[mk] += float(p.net_salary or 0)

        # ---- Congés ----
        leaves = Leave.query.all()
        leave_status = {"En attente": 0, "Approuvé": 0, "Rejeté": 0}
        for lv in leaves:
            s = lv.status or "En attente"
            if s in leave_status:
                leave_status[s] += 1

        # ---- Présences ----
        attendances = Attendance.query.all()
        total_att = len(attendances)
        absent_att = sum(1 for a in attendances if a.is_absent)
        absence_rate = round((absent_att / total_att * 100), 1) if total_att > 0 else 0

        # Présences par mois
        att_by_month: dict[str, int] = {m: 0 for m in last_12_months}
        abs_by_month: dict[str, int] = {m: 0 for m in last_12_months}
        for a in attendances:
            if a.check_in:
                mk = a.check_in.strftime("%Y-%m")
        ...
        for dept in departments:
            emp_list = [e for e in all_employees if e.department_id == dept.id]
            dept_names.append(dept.name)
            dept_counts.append(len(emp_list))
            dept_payroll.append(round(sum(float(p.net_salary or 0) for e in emp_list for p in e.payrolls), 2) if emp_list else 0)

        # ---- Formations ----
        trainings = Training.query.all()
        enrollments = TrainingEnrollment.query.all()
        completed_trainings = sum(1 for en in enrollments if en.status == "Terminé")
        in_progress_trainings = sum(1 for en in enrollments if en.status == "En cours")

        # ---- Évaluations ----
        evaluations = PerformanceEvaluation.query.all()
        avg_score = round(sum(float(ev.score or 0) for ev in evaluations) / len(evaluations), 1) if evaluations else 0
        score_dist = {"Excellent (9-10)": 0, "Bon (7-8)": 0, "Moyen (5-6)": 0, "Insuffisant (<5)": 0}
        for ev in evaluations:
            s = float(ev.score or 0)
            if s >= 9: score_dist["Excellent (9-10)"] += 1
            elif s >= 7: score_dist["Bon (7-8)"] += 1
            elif s >= 5: score_dist["Moyen (5-6)"] += 1
            else: score_dist["Insuffisant (<5)"] += 1

        # ---- Recrutement ----
        job_offers = JobOffer.query.all()
        applications = JobApplication.query.all()
        open_offers = sum(1 for j in job_offers if j.status == "Ouverte")
        app_status = {"Reçue": 0, "En cours": 0, "Retenu": 0, "Rejeté": 0}
        for app in applications:
            s = app.status or "Reçue"
            if s in app_status:
                app_status[s] += 1

        # ---- Contrats ----
        contracts = Contract.query.all()
        expiring_soon = sum(1 for c in contracts if c.end_date and first_day_month <= c.end_date <= today + timedelta(days=60))
        contract_types = {}
        for c in contracts:
            t = c.contract_type or "Autre"
        ...
        return {
            ...
            "payroll": {
                "total": round(total_payroll, 2),
                "average": avg_salary,
                "by_month": {
                    "labels": last_12_months,
                    "values": [round(payroll_by_month[m], 2) for m in last_12_months],
                },
            },
            "leaves": {
                "status": leave_status,
            ...
            "attendance": {
                ...
                "by_month": {
                    "labels": last_12_months,
                    "present": [att_by_month[m] - abs_by_month[m] for m in last_12_months],
                    "absent": [abs_by_month[m] for m in last_12_months],
                },
            },
            "departments": {
                "names": dept_names,
