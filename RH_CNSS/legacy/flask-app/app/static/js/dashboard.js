...
function chartColors(n, alpha = 0.85) {
  const base = [
    `rgba(79,127,255,${alpha})`,  `rgba(0,210,255,${alpha})`,
    `rgba(0,204,144,${alpha})`,   `rgba(255,176,32,${alpha})`,
    `rgba(255,77,109,${alpha})`,  `rgba(168,85,247,${alpha})`,
    `rgba(20,184,166,${alpha})`,  `rgba(236,72,153,${alpha})`,
    `rgba(99,102,241,${alpha})`,  `rgba(245,158,11,${alpha})`,
  ];
  const out = [];
      ...
      <tbody>${employees.map((e) => `
        <tr>
          ...
          <td>${e.has_fingerprint
            ? '<span class="badge badge-ok"><i class="ri-fingerprint-line"></i> Enrôlé</span>'
          ...
          <td>${e.rfid_card_id && e.rfid_card_active
            ? `<button class="btn btn-danger btn-sm" onclick="deactivateRfid(${e.id})"><i class="ri-sim-card-2-line"></i> Désactiver</button>`
            : ''}</td>
        </tr>`).join("")}
      </tbody>
    </table>`;
...
function setupBiometricActions() {
  ...
  if (enrollBtn) {
    enrollBtn.addEventListener("click", async () => {
      ...
      if (fpStatus) fpStatus.textContent = "Scan en cours — placez le doigt sur le lecteur...";
      enrollBtn.disabled = true;

      try {
        let template = null;

        // 1. Vérifier si le bridge est disponible via le proxy Flask
        if (fpStatus) fpStatus.textContent = "Vérification du lecteur...";
        const pingData = await api("/api/biometric/status").catch(() => {
          throw new Error("Service bridge biométrique non disponible");
        });
        if (!pingData.status || pingData.status !== "ok") throw new Error("Lecteur biométrique non connecté");

        // Scan réel via proxy Flask
        if (fpStatus) fpStatus.textContent = "Scan en cours — placez le doigt sur le lecteur...";
        const scanData = await api("/api/biometric/scan", {
          ...
          throw new Error(e.message || "Erreur bridge lors du scan");
        });
        if (!scanData.template) throw new Error("Aucun gabarit reçu du lecteur");
        template = scanData.template;

        if (fpStatus) fpStatus.textContent = "Empreinte capturée — enregistrement...";

        // 2. Enregistrer le template dans le backend
        ...
        if (fpStatus) fpStatus.textContent = "Enrôlement réussi !";
        notify("Empreinte enrôlée avec succès");
        await loadEnrolledEmployees();
      } catch (err) {
        ...
        setTimeout(() => {
          ...
          if (fpStatus) fpStatus.textContent = "Sélectionnez un employé et lancez l'enrôlement";
        }, 5000);
      }
    });
  }
