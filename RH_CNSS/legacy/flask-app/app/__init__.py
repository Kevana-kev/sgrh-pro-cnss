...
def _ensure_backward_compatible_schema() -> None:
    ...
    if "employees" in inspector.get_table_names():
        ...
        if "fingerprint_template" not in employee_columns:
            db.session.execute(text("ALTER TABLE employees ADD COLUMN fingerprint_template TEXT"))
        if "rfid_card_id" not in employee_columns:
            db.session.execute(text("ALTER TABLE employees ADD COLUMN rfid_card_id VARCHAR(64)"))
        if "rfid_card_active" not in employee_columns:
            db.session.execute(text("ALTER TABLE employees ADD COLUMN rfid_card_active BOOLEAN DEFAULT 1"))
    ...
    if "attendances" in inspector.get_table_names():
        ...
        if "source" not in att_columns:
            db.session.execute(text("ALTER TABLE attendances ADD COLUMN source VARCHAR(20) DEFAULT 'manual'"))
