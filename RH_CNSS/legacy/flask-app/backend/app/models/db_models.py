...
class Employee(db.Model):
    ...
    national_id = db.Column(db.String(40), nullable=True)
...
class Attendance(db.Model):
    ...
    # biometric_source: 'manual' | 'fingerprint' | 'rfid'
    biometric_source = db.Column(db.String(20), nullable=False, default="manual")
    ...
    device = db.relationship("BiometricDevice", back_populates="attendances")


class BiometricDevice(db.Model):
    """Représente un lecteur biométrique ou RFID enregistré dans le système."""
    __tablename__ = "biometric_devices"

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(120), nullable=False)
    device_type = db.Column(db.String(30), nullable=False, default="fingerprint")  # fingerprint | rfid | hybrid
    location = db.Column(db.String(120), nullable=True)
    is_active = db.Column(db.Boolean, nullable=False, default=True)
    bridge_url = db.Column(db.String(255), nullable=True)  # http://localhost:5002
    api_key_hash = db.Column(db.String(255), nullable=True)
    last_seen = db.Column(db.DateTime, nullable=True)
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)

    attendances = db.relationship("Attendance", back_populates="device", lazy=True)
...
class Leave(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    employee_id = db.Column(db.Integer, db.ForeignKey("employees.id"), nullable=False)
    leave_type = db.Column(db.String(40), nullable=False, default="Annuel")  # Annuel|Maladie|Maternité|Sans solde
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
    reason = db.Column(db.String(255), nullable=False)
    status = db.Column(db.String(40), nullable=False, default="En attente")  # En attente|Approuvé|Rejeté
