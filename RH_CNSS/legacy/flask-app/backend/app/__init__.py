...
# Resolve the frontend/ folder (sibling of backend/)
...
def _ensure_backward_compatible_schema() -> None:
    ...
    if "employees" in inspector.get_table_names():
        ...
        for col, typedef in [
            ...
            if col not in employee_columns:
                db.session.execute(text(f"ALTER TABLE employees ADD COLUMN {col} {typedef}"))
    ...
    if "leaves" in inspector.get_table_names():
        ...
        if "leave_type" not in leave_columns:
            db.session.execute(text("ALTER TABLE leaves ADD COLUMN leave_type VARCHAR(40) DEFAULT 'Annuel'"))
        if "approved_by_id" not in leave_columns:
            db.session.execute(text("ALTER TABLE leaves ADD COLUMN approved_by_id INTEGER"))

    if "attendances" in inspector.get_table_names():
        att_columns = {column["name"] for column in inspector.get_columns("attendances")}
        if "biometric_source" not in att_columns:
            db.session.execute(text("ALTER TABLE attendances ADD COLUMN biometric_source VARCHAR(20) DEFAULT 'manual'"))
        if "device_id" not in att_columns:
            db.session.execute(text("ALTER TABLE attendances ADD COLUMN device_id INTEGER"))
