...
public class HeadlessBridge
{
    ...
    private void LoadOrCreateApiKey()
    {
        try
        {
            var envKey = Environment.GetEnvironmentVariable("FINGERPRINT_BRIDGE_API_KEY");
            if (!string.IsNullOrWhiteSpace(envKey))
            {
                _apiKey = envKey.Trim();
                Log("Using API key from FINGERPRINT_BRIDGE_API_KEY.");
                return;
            }
    ...
    {
        var allowMock = Environment.GetEnvironmentVariable("FINGERPRINT_BRIDGE_ALLOW_MOCK") == "1";
        var forceDevice = Environment.GetEnvironmentVariable("FINGERPRINT_BRIDGE_FORCE_DEVICE") == "1";

        // Mode démo / tests sans lecteur branché
        if (allowMock && !forceDevice)
        {
            Log("ALLOW_MOCK=1 — returning mock fingerprint template (set FINGERPRINT_BRIDGE_FORCE_DEVICE=1 to use ZK SDK).");
            var mockQuick = Encoding.UTF8.GetBytes($"MOCK-FP-{DateTime.UtcNow:yyyyMMddHHmmss}-{Guid.NewGuid():N}");
            return Convert.ToBase64String(mockQuick);
        }

        // Prefer DLLs shipped next to the bridge (one-touch install), then SysWOW64
        ...
        if (!File.Exists(managedPath))
        {
            if (allowMock)
            {
                Log("SDK missing — returning mock fingerprint template (ALLOW_MOCK=1).");
                var mock = Encoding.UTF8.GetBytes($"MOCK-FP-{DateTime.UtcNow:yyyyMMddHHmmss}-{Guid.NewGuid():N}");
                return Convert.ToBase64String(mock);
            }
        ...
        try
        {
            ...
            if (devCount <= 0)
            {
                if (allowMock)
                {
                    Log("No device — returning mock fingerprint template (ALLOW_MOCK=1).");
                    var mock = Encoding.UTF8.GetBytes($"MOCK-FP-NODEV-{DateTime.UtcNow:yyyyMMddHHmmss}-{Guid.NewGuid():N}");
                    return Convert.ToBase64String(mock);
                }
...
internal static class NativeMethodsBootstrap
{
